...
describe('TriadsScreen', () => {
  ...
  test('renders title', () => {
    ...
    expect(json).toContain('Triads');
  });

  test('renders triad type options', () => {
    let tree: any;
    act(() => {
      tree = create(<TriadsScreen />);
    });
    ...
    expect(json).toContain('Aug');
  });

  test('renders string group picker', () => {
    let tree: any;
    act(() => {
      tree = create(<TriadsScreen />);
    });
  ...
  test('renders inversion options', () => {
    let tree: any;
    act(() => {
      tree = create(<TriadsScreen />);
    });
  ...
  test('selecting a note updates active note highlight', () => {
    ...
    // Press A (index 9 in NotePicker)
    act(() => { initialButtons[9].props.onPress(); });

    const updatedButtons = root.findAllByType('TouchableOpacity');
    const updatedStyle = [].concat(...updatedButtons[9].props.style);
  ...
  test('selecting a triad type updates active type highlight', () => {
    let tree: any;
    act(() => { tree = create(<TriadsScreen />); });
    const root = tree.root;

    // 0-11 NotePicker, 12-15 TypePicker (Major=12, Minor=13, Dim=14, Aug=15)
    const initialButtons = root.findAllByType('TouchableOpacity');
